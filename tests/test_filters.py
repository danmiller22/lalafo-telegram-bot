...
def test_channel_source_always_marks_unknown_author():
    ad = make_ad(
...
def test_stale_operator_priority_links_are_not_in_the_hot_path():
    assert PRIORITY_AD_SPECS == ()
...
def test_publish_batch_omits_realtor_author():
    ...
    assert "Автор:" not in format_apartment(realtor)
    assert author_label(realtor) is None


def test_unknown_author_label_is_stable_and_exact():
    ads = [
        make_ad(owner_listing=False, seller_type="unknown", phone=f"+996700{index:06d}")
