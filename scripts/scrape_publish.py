...
def apartment_to_ad(apartment) -> LalafoAd:
    ...
    return LalafoAd(
        ...
        district=apartment.district or ("Центр" if apartment.price >= 25_000 else None),
        city=apartment.city,
...
async def run(*, discovery_only: bool = False) -> int:
    ...
    async with AsyncExitStack() as stack:
        ...
        for priority_url, district_label in PRIORITY_AD_SPECS:
            ...
            # Persist each fresh direct card immediately instead of waiting
            # for every priority URL and the complete catalogue crawl.
            if discovery_only:
                ...
                logger.info(
                    "Priority inventory scheduled immediately: id=%s queued=%d",
                    priority_id,
                    early_queued,
                )
