...
FAQ_ITEMS = (
    ...
    FAQItem(
        ...
        answer=(
            ...
            "После оплаты нажмите «Получить номер» — карточка выдаётся автоматически."
        ),
    ),
    FAQItem(
        key="review",
        ...
        answer=(
            "❓ Вернитесь к экрану оплаты и нажмите «Получить номер». Карточка с "
    ...
    FAQItem(
        ...
        answer=(
            "🏠 Нажмите «Проверить актуальность» под карточкой. Бот проверит исходное "
            "объявление. Даже при активной ссылке окончательно уточните доступность по телефону."
        ),
    ),
    FAQItem(
        key="catalog",
    ...
    FAQItem(
        ...
        answer=(
            ...
            "Проверьте личность, право сдавать квартиру и оригиналы документов, "
            "заключите письменный договор. Никому не сообщайте коды из SMS."
        ),
    ),
    FAQItem(
        key="authors",
        button="👤 Что означает автор?",
        phrases=("автор собственник", "автор агент", "автор неизвестно", "метка автора"),
        answer=(
            "👤 Метка строится по данным источника и словам объявления: «собственник», "
            "«агент» или «неизвестно». Arenda.KG не проверяет личность автора, поэтому "
            "всегда проверяйте документы и не переводите предоплату до просмотра."
        ),
    ),
    FAQItem(
        key="refund",
