...
class TokenSigner:
    ...
    @staticmethod
    def decode_public_start_id(token: str) -> int | None:
        """Read the public object ID from a Telegram start token.

        Apartment cards are public and can be generated by a separate cloud
        worker.  Decoding their ID must not depend on that worker sharing the
        bot's callback-signing secret.  Sensitive callback actions still use
        the verified signatures above.
        """
