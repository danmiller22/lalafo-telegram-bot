...
2. В личном чате бот покажет тарифы: 7 дней за 499 сом и 30 дней за 999 сом.
3. Оплатите по персональной ссылке Finik.
4. Finik отправляет подписанный webhook, бот проверяет подпись и сумму.
5. Доступ активируется автоматически на 7 или 30 дней. Повторный webhook
   безопасен и не продлевает подписку второй раз.

Для тарифа на 30 дней задайте отдельную платёжную ссылку Finik в
`MONTHLY_FINIK_PAYMENT_URL`; цена платёжной ссылки в кабинете Finik должна быть 999 сом.

Для автоматической выдачи задайте секреты `FINIK_API_KEY`, `FINIK_ACCOUNT_ID` и
`FINIK_PRIVATE_KEY_B64`. Endpoint должен быть
`https://api.acquiring.averspay.kg/v1/payment`; callback создаётся автоматически как
`<PUBLIC_BASE_URL>/finik/webhook`. Пока эти секреты не заданы, остаётся безопасный
ручной flow по статическим QR-ссылкам.
...
- `LALAFO_AUTO_REPLY_POLL_SECONDS=10`. Значение меньше десяти секунд
  ...
  Telegram-каналом, очередь ограничена пятью ответами за проход и ста ответами
  за сутки, а одному диалогу бот отвечает не чаще одного раза в 24 часа.
  Авторизованный вход никогда не переключается на случайные публичные прокси.

Единственный авторизованный Lalafo-сеанс работает внутри Koyeb. Старый
Semaphore-запуск больше не входит в аккаунт и не отправляет сообщения: он
только проверяет `/health`. Это исключает параллельные входы с разных облачных
IP и конфликтующие автоответы.
